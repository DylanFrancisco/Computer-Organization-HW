...
void parse(char* line, command_t* p_cmd) {
    ...
    p_cmd->argc = 0; //init for p_cmd
    int i = 0;
    char *token = strtok(line, " ");
    char **temp[MAX_ARG_LEN][MAX_ARG_LEN];
    while (token != NULL){
        strcpy(temp[i], token);
        token = strtok(NULL, " ");
        i++;
...
int find_fullpath( command_t* p_cmd ) {
    ...
    char *folder = strtok(path, PATH_SEPARATOR);

    while (folder != NULL) {
        char fullpath[MAX_ARG_LEN];
        strcpy(fullpath, folder);
        strcat(fullpath, "/");
        strcat(fullpath, p_cmd->argv[0]);

        struct stat buffer;
        if (stat(fullpath, &buffer) == 0 && (S_IFREG & buffer.st_mode)) {
            strcpy(p_cmd->argv[0], fullpath);
            return TRUE;
        }
        folder = strtok(NULL, PATH_SEPARATOR);
    }
...
int execute( command_t* p_cmd ) {
    ...
    if (is_builtin(p_cmd)) {
        ...
        else if (child_pid == 0) { //if <0 do nothing
